...
var wode = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAGklEQVRYhe3BAQEAAACCIP+vbkhAAQAAAO8GECAAAUcBoIgAAAAASUVORK5CYII=";
!(function ($) {
    if (!$.sessionStorage.get("gt")) {
        $.sessionStorage.set("gt","CE44712FD1C41F149612F9A97E3D99D51E336E5FC21AF13257DEA04F592130F41B58BFE891CF45309867E31111440630");
        $.sessionStorage.set("lt",new Date().getTime());
    }
    ...
    $.download_XMLHttpRequest = function (url, fn, data, method) { // 获得url和data
        ...
        // 设置请求头
        xhr.setRequestHeader('fileUrl', $('#downloadUrl').attr('href'));
        // 发送ajax请求
        xhr.send(form);
    ...
    function sendReq(link) {
        var param = {"timestamp": new Date().getTime(), "Nonce-Str": randomString()}
        var dataParam = {
            "url": link,
            "uid": document.getElementById('uId').value, "timestamp": param.timestamp, "nonce_str": param["Nonce-Str"]
        };
        $.ajax({
            headers: {
                "sign": getSign(dataParam),
                "timestamp": param.timestamp,
                "Nonce-Str": param["Nonce-Str"],
                "packageName":"website",
                "Os-Version":navigator.userAgent,
                "Os-Device-Brand":navigator.oscpu,
                "Os-Model":(navigator.language || navigator.browserLanguage).toLowerCase()
            },
            type: "POST",
            url: "../nowater/videoParse/webParse",
            // url: "../nowater/videoParse/webParse",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({"url": dataParam.url, "uid": dataParam.uid}),
            dataType: "json",
            success: function (data) {
                if (data.code != 0) {
                    alert.innerHTML = "Error," + data.detail + " something went wrong.";
                    trackEvent('parseError', document.getElementById("url").value);
                    notifyAction();
                    if (data.code == 3041){
                        if(confirm("Your IP checking times exceeds the daily system limit.Download app to get more check times!")){
                            window.open("https://play.google.com/store/apps/details?id=com.downloadvideotiktok.nowatermark")
                        }
                    }
                } else {
                    var html = '<div class="row">\n' +
                        '    <div class="col-12">\n' +
                        ' <video id="video" style="max-height: 400px;max-width: 100%;" preload="auto" controls="" controlslist="nodownload" controls="controls" src="' + data.response.videoInfo.url + '"></video>' +
                        '        <div class="videottdownloader mb-10">\n' +
                        '            <div class="videottdownloader-left left">\n' +
                        '                <img id="cover" src="' + data.response.videoInfo.cover + '" class="" alt="">\n' +
                        '            </div>\n' +

                        '            <div class="videottdownloader-right is-desktop-only right" style="width: 250px;">\n' +
                        '             \n' +
                        '                <div class="abuttons mb-0">\n' +
                        '                    <a onclick="$.download_XMLHttpRequest(\'https://tiktok.datidahui.com/downloadFile\', \'video.mp4\', \'token=SCBC#\', \'post\');"  href="javascript:;" class="abutton is-success is-fullwidth" rel="nofollow" target="_blank"><span><span>Download Server (Direct  File)</span></span></a>\n' +
                        '                </div>\n' +
                        '            </div>\n' +
                        '            <div class="videottdownloader-right is-desktop-only right" style="width: 250px;">\n' +
                        '             \n' +
                        '                <div class="abuttons mb-0">\n' +
                        '                    <a onclick="trackEvent(\'send\', \'event\', \'home\', \'download_video_Source\');" id="downloadUrl" href="' + data.response.videoInfo.url + '" class="abutton is-success is-fullwidth" rel="nofollow" target="_blank"><span><span>Download Server (Video Source)</span></span></a>\n' +
                        '                </div>\n' +
                        '            </div>\n' +
                        ...
                        '             \n' +
                        '                <div class="abuttons mb-0">\n' +
                        '                   <a href="" onclick="javascript:location.reload();"  rel="nofollow" class="abutton is-success is-fullwidth">\n' +
                        '                            Back Home\n' +
                        '                        </a>\n' +
                        '                </div>\n' +
                        '            </div>\n' +

                        '            <div class="videottdownloader-middle center">\n' +
                        '                <div>\n' +
                        '                    <h1 itemprop="name"><div class="hover-underline" style="color:#FFFFFF ;" title="' + data.response.text + '" id="title">' + data.response.text + '</div></h1>\n' +
                        '                </div>\n' +
                        '                <p><span></span></p>\n' +
                        '            </div>\n' +
                        '        </div>\n' +
                        ...
                        '    </div>\n' +
                        '</div>';
                    $("#download-section").html(html);
                    $("#inputData").remove();
                    $("#why-section").remove();
                    $(".content-box").remove();
                    trackEvent('parseSuccess');
                }
            },
            error: function (xhr, err, exception) {
                alert.innerHTML = "Error," + err + " something went wrong.";
                notifyAction();
            }
        });
    }
    ...
    function pasteLink(isReturn) {
        ...
        '<span><i class="icon icon-clear"></i>Clear</span>' === a.innerHTML ? (c.value = "", a.innerHTML = '<span><i class="icon icon-paste"></i>Paste</span>') :
            ...
            navigator.clipboard.readText().then(function (f) {
                    ...
                    var twitter = /.*https?:\/\/.*twitter\.com\/.*/ig;
                    var mxtakatak = /.*https?:\/\/.*mxtakatak\.com\/.*/ig;
                    console.log("/n链接内容" + f);
                    var isTikLink = regTik.test(f);
                    var isDouyinLink = regDouyin.test(f);
                    var isTwitterLink = twitter.test(f);
                    var isMxtakatakLink = mxtakatak.test(f);
                    console.log("/n格式" + isTikLink);
                    console.log("/n格式" + isDouyinLink);

                    if (!isTikLink && !isDouyinLink && !isTwitterLink && !isMxtakatakLink) {
                        console.log("/n链接格式错误");
                        isTikLink = false;
                        isDouyinLink = false;
                        //格式错误
                        alert.innerHTML = "Error, The  link is temporarily invalid.";
                        notifyAction();
                        if (isReturn) {
                            return c.value = "";
                        } else {
    ...
    $("#send").click(function () {
        ...
        if (idx != -1){
            link = link.substr(0,idx)
        }
        console.log("/n链接内容" + link);
        if (!link) {
            alert.innerHTML = "Error, Video Link Not Null";
            notifyAction();
            ...
            var mxtakatak = /.*https?:\/\/.*mxtakatak\.com\/.*/ig;
            console.log("/n链接内容" + link);
            var isTikLink = regTik.test(link);
            var isDouyinLink = regDouyin.test(link);
            var isTwitterLink = twitter.test(link);
            var isMxtakatakLink = mxtakatak.test(link);;
            console.log("/n格式" + isTikLink);
            console.log("/n格式" + isDouyinLink);
    ...
    function paramsStrSort(paramsStr) {
        var result = Decrypt($.sessionStorage.get("gt"));
        var url = paramsStr;
        var urlStr = url.split("&").sort().join("&");
        var newUrl = urlStr + '&key=' + wode;
        var sign = CryptoJS.HmacSHA256(newUrl, wode).toString();
   ...
   function getUID(fn,link){
       var param = {"timestamp": new Date().getTime(), "Nonce-Str": randomString()}
       var dataParam = {
           "timestamp": param.timestamp, "nonce_str": param["Nonce-Str"]
       };
       $.ajax({
           headers: {"sign": getSign(dataParam), "timestamp": param.timestamp, "Nonce-Str": param["Nonce-Str"],"packageName":"website"},
           type: "POST",
           url: "../nowater/videoParse/getUid",
           // url: "../nowater/videoParse/getUid",
           contentType: "application/json; charset=utf-8",
           dataType: "json",
           success: function (data) {
               if (data.code === 0) {
                   document.getElementById('uId').value = data.response.uid
                   fn(link);
               } else {
                   alert.innerHTML = "Error," + data.detail + " something went wrong.";
                   notifyAction();
    ...
    var key = CryptoJS.enc.Utf8.parse("5B97LQS0LH4REBAZ");
    var iv = CryptoJS.enc.Utf8.parse('BL1LMU8UKFTNHF26');
    ...
    $("#bookmark").click(function () {
    	trackEvent('click_bookmark');
    	
    	var add_bookmark_tips_1 = {
    		"en": "Press Ctrl+D to bookmark this site.",
    		"ar": "اضغط على Ctrl + D لوضع إشارة مرجعية على هذا الموقع.",
    		"de": "Drücken Sie Strg+D, um diese Seite mit einem Lesezeichen zu versehen.",
    		"es": "Presione Ctrl+D para marcar este sitio.",
    		"fa": "Ctrl+D را فشار دهید تا این سایت را نشانه گذاری کنید.",
    		"fr": "Appuyez sur Ctrl+D pour mettre ce site en signet.",
    		"id": "Tekan Ctrl+D untuk menandai situs ini.",
    		"it": "Premi Ctrl+D per aggiungere questo sito ai segnalibri.",
    		"ja": "このサイトをブックマークするには、Ctrl+D を押してください。",
    		"ko": "이 사이트를 북마크하려면 Ctrl+D를 누르십시오.",
    		"pt": "Pressione Ctrl+D para marcar este site como favorito.",
    		"ru": "Нажмите Ctrl+D, чтобы добавить этот сайт в закладки.",
    		"th": "กด Ctrl+D เพื่อบุ๊กมาร์กไซต์นี้",
    		"tr": "Bu siteye yer işareti koymak için Ctrl+D tuşlarına basın.",
    		"vi": "Nhấn Ctrl + D để đánh dấu trang web này.",
    		"zh-CN": "按 Ctrl+D 组合键将本网站加入书签。",
    		"zh-TW": "按 Ctrl+D 組合鍵將本網站加入書籤。"
    	};
    	var add_bookmark_tips_2 = {
        	"en": "Press Cmd+D to bookmark this site.",
        	"ar": "اضغط على Cmd + D لوضع إشارة مرجعية على هذا الموقع.",
    		"de": "Drücken Sie Cmd+D, um diese Seite mit einem Lesezeichen zu versehen.",
    		"es": "Presione Cmd+D para marcar este sitio.",
    		"fa": "برای نشانک کردن این سایت، Cmd+D را فشار دهید.",
    		"fr": "Appuyez sur Cmd+D pour mettre ce site en signet.",
    		"id": "Tekan Cmd+D untuk menandai situs ini.",
    		"it": "Premi Cmd+D per aggiungere questo sito ai segnalibri.",
    		"ja": "このサイトをブックマークするには、Cmd+D を押してください。",
    		"ko": "이 사이트를 북마크하려면 Cmd+D를 누르십시오.",
    		"pt": "Pressione Cmd+D para marcar este site como favorito.",
    		"ru": "Нажмите Cmd+D, чтобы добавить этот сайт в закладки.",
    		"th": "กด Cmd+D เพื่อบุ๊กมาร์กไซต์นี้",
    		"tr": "Bu siteye yer işareti koymak için Cmd+D tuşlarına basın.",
    		"vi": "Nhấn Cmd+D để đánh dấu trang web này.",
        	"zh-CN": "按 Cmd+D 组合键将本网站加入书签。",
        	"zh-TW": "按 Cmd+D 組合鍵將本網站加入書籤。"
        };
    	...
    	if (pathname.length > 1) {
    		lan = pathname.substring(1, pathname.substr(1).indexOf('/') + 1);
    		if (add_bookmark_tips_1[lan] == undefined || add_bookmark_tips_2[lan] == undefined) {
    			lan = "en";
    		}
    	}
    	
    	var tips = /Mac/i.test(navigator.userAgent) ? add_bookmark_tips_2[lan] : add_bookmark_tips_1[lan];
    	window.alert(tips);
    });
